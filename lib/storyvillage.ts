...
// ✅ [원본] 기획자님 원본 태그 리스트 100% 복구 (기획자님의 #태그 그대로)
...
export const TAG_GROUPS: Record<string, string[]> = {
  ...
  '🧱 핵심 소재': ['#회귀', '#빙의', '#환생', '#현판', '#무협', '#판타지', '#헌터물', '#탑등반', '#아카데미', '#게임빙의', '#대체역사', '#전문직', '#연예계', '#재벌', '#스포츠'],
  '💖 관계/로맨스': ['#하렘', '#역하렘', '#브로맨스', '#워맨스', '#집착', '#후회', '#순애', '#티키타카', '#육아물', '#복수'],
  '📌 상태/기타': ['#완결', '#연재중', '#장편', '#단편', '#웹툰화'],
};

// ✅ [원본] 타입 정의 및 기본 설정 유지
export type TrustBadge = '⚪' | '🟢' | '🟡' | '🔴';
export type TasteKey = 'cider' | 'pace' | 'dark' | 'romance' | 'probability' | 'character' | 'growth';
...
// 🔧 [로컬 연동 픽스] page - app.tsx 에서 누락되어 에러나던 함수만 보강
export function recommendCoreTagsFromTaste(t: Taste): string[] {
  const tags: string[] = [];
  if (t.cider >= 70) tags.push('사이다');
  if (t.cider <= 30) tags.push('고구마');
  if (t.pace >= 70) tags.push('빠른전개');
  if (t.pace <= 30) tags.push('느린전개');
  if (t.dark >= 70) tags.push('피폐');
  if (t.dark <= 30) tags.push('힐링');
  if (t.romance <= 20) tags.push('노맨스');
  if (t.romance >= 80) tags.push('순애');
  if (t.character >= 70) tags.push('악당형');
  if (t.growth >= 70) tags.push('먼치킨');
  if (t.growth <= 30) tags.push('성장형');
  if (t.probability >= 70) tags.push('지능캐');
  return tags.slice(0, 3);
}

// ✅ [원본] 검색 및Preset 로직 100% 복구
export function buildSearchUrl(args: { tags: string[]; taste: Taste; mode?: string }) {
  const p = new URLSearchParams();
  if (args.tags.length) p.set('tags', args.tags.map(encodeURIComponent).join(','));
  p.set('cider', String(Math.round(args.taste.cider)));
  p.set('pace', String(Math.round(args.taste.pace)));
  p.set('dark', String(Math.round(args.taste.dark)));
  p.set('romance', String(Math.round(args.taste.romance)));
  p.set('probability', String(Math.round(args.taste.probability)));
  p.set('character', String(Math.round(args.taste.character)));
  p.set('growth', String(Math.round(args.taste.growth)));
...
export function recommendPresetFromTags(tags: string[]): { label: string; delta: Partial<Taste> } | null {
  ...
  const label: string[] = [];
  if (has('#사이다') || has('#통쾌함')) { delta.cider = (delta.cider ?? 0) + 30; label.push('사이다↑'); }
  if (has('#고구마')) { delta.cider = (delta.cider ?? 0) - 30; label.push('고구마(사이다↓)'); }
  if (has('#빠른전개')) { delta.pace = (delta.pace ?? 0) + 30; label.push('전개↑'); }
  if (has('#느린전개') || has('#잔잔함')) { delta.pace = (delta.pace ?? 0) - 30; label.push('잔잔↑'); }
  if (has('#피폐') || has('#다크') || has('#광기')) { delta.dark = (delta.dark ?? 0) + 30; label.push('다크↑'); }
  if (has('#힐링') || has('#개그')) { delta.dark = (delta.dark ?? 0) - 30; label.push('힐링↑'); }
  if (has('#노맨스')) { delta.romance = (delta.romance ?? 0) - 40; label.push('로맨스↓'); }
  if (has('#순애') || has('#하렘') || has('#역하렘')) { delta.romance = (delta.romance ?? 0) + 40; label.push('로맨스↑'); }
  if (has('#먼치킨') || has('#힘숨김')) { delta.growth = (delta.growth ?? 0) + 30; label.push('완성형↑'); }
  if (has('#성장형')) { delta.growth = (delta.growth ?? 0) - 30; label.push('성장형↑'); }
  if (has('#지능캐')) { delta.probability = (delta.probability ?? 0) + 20; label.push('지능캐(개연성↑)'); }
...
export function calcTasteMatch(userTaste: Taste, adminTaste: Taste) {
  const keys: (keyof Taste)[] = ['cider', 'pace', 'dark', 'romance', 'probability', 'character', 'growth'];
